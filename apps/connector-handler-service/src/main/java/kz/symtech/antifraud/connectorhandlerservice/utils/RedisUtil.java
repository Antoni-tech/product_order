...
public class RedisUtil {
    ...
    public Boolean putValue(String hashKey, String key, Object value) {
        try {
    ...
    public void clearByHashKeyAndKeys(String hashKey, List<String> keys) {
        try {
            hashOperations.delete(hashKey, keys);
        } catch (Exception e) {
            log.error(String.format("Can't delete object(s) due to %s", e.getMessage()));
        }
    }
    ...
    public void increment(String key) {
        try {
            ...
            log.error(String.format("Can't increment counters due to %s", e.getMessage()));
        }
    }

    public Boolean increment(String hashKey, String key, Long delta) {
        try {
            hashOperations.increment(hashKey, key, delta);
            return true;
        } catch (Exception e) {
            log.error(String.format("Can't increment counters due to %s", e.getMessage()));
            return false;
